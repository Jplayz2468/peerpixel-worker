...
<section class="card" id="progressPanel"><div class="row"><b id="progressText">Waiting for work</b><span class="muted" id="elapsed">0s</span></div><div class="bar"><div class="fill" id="fill"></div></div></section>
...
async function showWork(){try{const s=await call('/api/state'),working=['loading','benchmarking','downloading'].includes(s.phase);$('fill').classList.toggle('working',working);if(working){$('state').textContent='Working…';$('phase').textContent=s.phase==='downloading'?'Downloading model…':s.phase==='loading'?'Loading model…':'Warming up and benchmarking…';$('progressText').textContent='Working, this is not stuck';$('prompt').textContent='This can take several minutes the first time. It is working.'}else if(s.lastImageAt&&Date.now()-s.lastImageAt<15000){$('progressText').textContent=s.lastEarnedPixels>0?`Completed · +${s.lastEarnedPixels} px`:'Completed · self/free render, no payout';$('fill').style.width='100%'}}catch{}}refresh();showWork();setInterval(()=>{refresh();showWork()},1000)
