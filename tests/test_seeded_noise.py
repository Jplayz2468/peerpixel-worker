...
class ShapeTests(unittest.TestCase):
    ...
    def test_the_final_and_the_preview_are_the_documented_sizes(self):
        # 1024/16 = 64 and 256/16 = 16, which is the exact factor of four the
        # averaging depends on.
        self.assertEqual(latent_grid(PIPE, OPERATIONS["master"]["width"]), 64)
        self.assertEqual(latent_grid(PIPE, OPERATIONS["draft"]["width"]), 16)
...
class SeedTests(unittest.TestCase):
    ...
    def test_the_preview_is_the_final_averaged_and_not_a_separate_draw(self):
        ...
        expected = final.reshape(1, 128, 16, 4, 16, 4).mean(dim=(3, 5)) * 4
